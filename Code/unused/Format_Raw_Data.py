...
def main():
    ...
    data_2014 = np.genfromtxt(
        ...
        usecols=(1, 9, 10, 4, 5, 12, 15, 14, 11),
        filling_values="NA",
        skip_header=1
    )
    ...
    data_2015 = np.genfromtxt(
        ...
        delimiter=",",
        dtype=(str, Constants.STR_LENGTH),
        usecols=(1, 14, 15, 6, 7, 17, 20, 19, 16),
        filling_values="NA",
        skip_header=1
    )
...
# This matrix creates and returns the numpy label vector for a particular data set. The labels are reformatted to be
# binary: -1 for no algae and 1 for algae
    ...
    if not summary:
        for i in range(0, mat.shape[Constants.ROWS]):
            if ("NA" not in mat[i, :] and "" not in mat[i, :] and "FALSE" not in mat[i, :] and
                ...
                new_mat[idx, Constants.LOCATION] = mat[i, 0]                        # Locations
                new_mat[idx, Constants.DATE] = mat[i, 1]                            # Date
                new_mat[idx, Constants.TIME] = mat[i, 2]                            # Time (24-hour)
                new_mat[idx, Constants.ALGAL_BLOOMS] = mat[i, 3]                    # algalBlooms
                new_mat[idx, Constants.ALGAL_BLOOM_SHEEN] = mat[i, 4]               # algalBloomSheen
                new_mat[idx, Constants.WATER_APPEARANCE] = mat[i, 5]                # waterAppearance
                new_mat[idx, Constants.WAVE_INTENSITY] = mat[i, 6]                  # waveIntensity
                new_mat[idx, Constants.WATER_TEMP] = mat[i, 7]                      # waterTemp
                new_mat[idx, Constants.TURBIDITY] = mat[i, 8]                       # turbidity

                # if the qualitative measurement does not conform to the convention for quantization, delete the sample
                for j in range(Constants.WATER_APPEARANCE, Constants.WATER_TEMP):
...
def build_summer_months(mat):
    ...
    for i in range(0, mat.shape[Constants.ROWS]):
        try:
            curr_month = int(mat[i, Constants.DATE][0:2])
        except ValueError:
            curr_month = int(mat[i, Constants.DATE][0:1])
