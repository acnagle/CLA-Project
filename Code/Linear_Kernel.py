...
def main():
    ...
    for i in range(0, len(data_vec)):
        ...
        for k in range(0, c.shape[0]):
            ...
            print_results(
                title="Results for " + data_desc[i] + " (Kernel type: Linear, C = " + str(c[k]) + ")",
                ber=total_ber,
        ...
        plt.title("\n".join(wrap("Error Rates vs. C for " + data_desc[i] + " (Kernel type: Linear)", 60)))
        plt.savefig(os.path.join(dest_path, "Error Rates vs. C for " + data_desc[i] + " (Linear Kernel).png"))
